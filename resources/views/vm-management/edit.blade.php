...
@section('breadcrumb', 'Administration / VM Management / Edit')
...
<div class="card border-0 shadow-sm">
    <div class="card-header bg-white border-0 pt-4">
        <h3 class="card-title" style="color:#1a3c6b">Edit VM — {{ $vm->vm_name }}</h3>
    </div>
    <div class="card-body">
        <form method="POST" action="{{ route('admin.vm-management.update', $vm) }}">
            @csrf @method('PUT')
            <div class="row g-3">
                {{-- Basic Info --}}
                <div class="col-md-6">
                    ...
                    <input type="text" name="vm_name" class="form-control @error('vm_name') is-invalid @enderror"
                           value="{{ old('vm_name', $vm->vm_name) }}" required>
                ...
                <div class="col-md-6">
                    ...
                    <input type="text" name="application_name" class="form-control @error('application_name') is-invalid @enderror"
                           value="{{ old('application_name', $vm->application_name) }}" required>
                ...
                <div class="col-md-4">
                    ...
                    <input type="text" name="os_type" class="form-control @error('os_type') is-invalid @enderror"
                           value="{{ old('os_type', $vm->os_type) }}" required>
                    @error('os_type')<div class="invalid-feedback">{{ $message }}</div>@enderror
                </div>
                <div class="col-md-4">
                    <label class="form-label fw-semibold">Status *</label>
                    <select name="status" class="form-select @error('status') is-invalid @enderror">
                        @foreach(['stopped','running','paused'] as $s)
                        <option value="{{ $s }}" {{ old('status', $vm->status)==$s?'selected':'' }}>{{ ucfirst($s) }}</option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-4">
                    ...
                    <select name="assigned_user_id" class="form-select">
                        ...
                        <option value="{{ $user->id }}" {{ old('assigned_user_id', $vm->assigned_user_id)==$user->id?'selected':'' }}>{{ $user->name }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="col-md-4">
                    ...
                    <input type="text" name="region" class="form-control" value="{{ old('region', $vm->region) }}">
                </div>
                <div class="col-md-4">
                    <label class="form-label fw-semibold">Data Center</label>
                    <input type="text" name="data_center" class="form-control" value="{{ old('data_center', $vm->data_center) }}">
                </div>
                <div class="col-md-4">
                    <label class="form-label fw-semibold">IP Address</label>
                    <input type="text" name="ip_address" class="form-control" value="{{ old('ip_address', $vm->ip_address) }}">
                </div>
                <div class="col-md-4">
                    <label class="form-label fw-semibold">Host Server</label>
                    <input type="text" name="host_server" class="form-control" value="{{ old('host_server', $vm->host_server) }}">
                </div>
                <div class="col-md-4">
                    <label class="form-label fw-semibold">CPU Cores</label>
                    <input type="number" name="cpu_cores" class="form-control" value="{{ old('cpu_cores', $vm->cpu_cores) }}" min="1">
                </div>
                <div class="col-md-4">
                    <label class="form-label fw-semibold">RAM (GB)</label>
                    <input type="number" name="ram_gb" class="form-control" value="{{ old('ram_gb', $vm->ram_gb) }}" min="1">
                </div>
                ...
                <div class="col-md-4">
                    ...
                    <div class="form-check form-switch mt-2">
                        ...
                        <input class="form-check-input" type="checkbox" name="has_gpu" value="1"
                               id="has_gpu" {{ old('has_gpu', $vm->has_gpu) ? 'checked' : '' }}>
                        <label class="form-check-label" for="has_gpu">Has GPU</label>
                    </div>
                </div>
                <div class="col-md-8">
                    <label class="form-label fw-semibold">GPU Model</label>
                    <input type="text" name="gpu_model" class="form-control" value="{{ old('gpu_model', $vm->gpu_model) }}">
                </div>

                {{-- Notes --}}
                <div class="col-12">
                    <label class="form-label fw-semibold">Notes</label>
                    <textarea name="notes" class="form-control" rows="2">{{ old('notes', $vm->notes) }}</textarea>
                </div>

                <div class="col-12 d-flex gap-2">
                    <button type="submit" class="btn" style="background:#1a3c6b;color:#fff">Update VM</button>
